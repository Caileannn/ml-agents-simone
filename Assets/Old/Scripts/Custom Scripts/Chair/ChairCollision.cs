...
public class ChairCollision : Agent
{
    // Walking Speed
    [Header("Walk Speed")]
    [Range(0.1f, 10)]
    ...
    // Spawn Obstacles
    SpawnObstacles m_SpawnObstacles;

    public string obstacleTag = "obst";
    ...
    public override void Initialize()
    {
        ...
        m_SpawnObstacles = GetComponentInParent<SpawnObstacles>();
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        bpDict[BRF].SetJointTargetRotation(continuousActions[++element], continuousActions[++element], continuousActions[++element]);
        bpDict[BLF].SetJointTargetRotation(continuousActions[++element], continuousActions[++element], continuousActions[++element]);
        bpDict[FRF].SetJointTargetRotation(continuousActions[++element], continuousActions[++element], continuousActions[++element]);
        bpDict[FLF].SetJointTargetRotation(continuousActions[++element], continuousActions[++element], continuousActions[++element]);
    ...
    void FixedUpdate()
    {
        ...
        // Distance from Target, approaches zero as the agent deviates
        var distFromTarget = Mathf.Abs(Vector3.Distance(m_OrientationCube.transform.position, target.position));
        ...
        // Inverse Distance from Target * Matching Velocity + (0.1 * look@Target)

        // AddReward(distFromTarget * matchSpeedReward);

        // AddReward(0.1f * lookAtTargetReward);

        AddReward(matchSpeedReward * lookAtTargetReward);

        // AddReward(matchSpeedReward * lookAtTargetReward);
        //if (checkIfTouchingObst())
        //{
        //    AddReward(-1.0f);
        //}
    }
    ...
    public bool checkIfTouchingObst() 
    {
        foreach (var bodyPart in m_JointDriveController.bodyPartsList)
        {
